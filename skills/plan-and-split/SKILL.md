...
> **面向 agentic workers：** REQUIRED SUB-SKILL: 使用 subagent-driven-development 逐个任务实施此计划。步骤使用 checkbox (`- [ ]`) 语法进行跟踪。
...
完成 `tasks.md` 后，向用户声明："计划已完成并保存到 `.comate/specs/{feature_name}/tasks.md`。开始 subagent 驱动的执行流程，每个任务派发一个新的 subagent，并在任务之间进行审查。"。并调用 `subagent-driven-development` skill 进行开发流程：
