...
基于 `split` 已创建并经用户确认的 Feature / Story，生成可由 `execute-task-plan` 执行的 `.comate/specs/{feature_name}/tasks.md`
...
flowchart TD
    ...
    F --> G{Question: 用户是否确认?}
    G -->|修改计划| C
    G -->|确认计划| H((调用 execute-task-plan skill))
```
