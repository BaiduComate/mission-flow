...
展示当前理解后，**必须**使用 question 工具询问用户是否进行方案设计。面向用户不得直接使用 `design`、`split` 等内部 skill 名称。问题必须包含两个选项：

- 进行方案设计：先编写并评审设计文档，再拆分 iCafe 卡片。选择后内部调用 `design`
- 直接拆分卡片：跳过设计文档，直接创建 iCafe Feature 和 Story。选择后内部调用 `split`
...
flowchart TD
    ...
    D --> E
    E --> F{是否进行方案设计?}
    F -->|进行方案设计| G((编写并评审设计文档))
    F -->|直接拆分卡片| H((创建 iCafe 研发卡片))
```
