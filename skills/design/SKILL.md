...
name: design
description: |-
  仅在需求澄清完成后，且用户通过 question 明确选择进行方案设计时使用，用于编写 `doc.md` 设计文档。
  如果用户直接要求设计文档但当前会话没有需求澄清上下文，必须先调用 think skill
metadata:
  version: 0.2.1
---
...
**开始时声明：**“已进入方案设计阶段，我会补充技术调研、编写设计文档并完成自审。”不得向用户展示 `design` 这一内部 skill 名称。

`design` 只解决方案不确定性，不创建 iCafe 卡片，不编写计划文档，不编写代码。
...
将 `think` 上下文、iCafe 上下文和 explore 子 agent 报告合并，写入：
...
审查通过后，必须使用 question 工具请求用户审阅文档：
