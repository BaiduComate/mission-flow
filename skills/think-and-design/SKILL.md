...
description: 必须在任何创造性工作之前使用，比如创建功能、构建组件、添加能力或修改当前代码行为
---
...
> [!IMPORTANT]
> 无论项目看起来多简单，在你展示设计并获得用户批准之前，**不要**调用任何实现类 skill 或者编写代码。
...
1. 探索项目上下文
2. 提出澄清问题：**必须**使用 question 工具提问，将独立问题合并到一条消息中；只有当后续问题依赖先前答案时才拆成多轮提问。
3. 提出 2-3 个方案：**必须**使用 question 工具提问，每个问题都要包含你的推荐项，每个问题的条目都要包含为什么给出该选项的权衡，等待用户选择或者修正。
4. 展示设计
5. 编写设计文档：将已验证的设计写入 `.comate/specs/{feature_name}/doc.md`
6. 派发子 agent 审查设计文档
7. 用户审查已写好的 `doc.md`
8. 使用 `plan-and-split` 过渡到实现
...
调用 `plan-and-split` skill 创建详细实现计划。不要调用任何其他 skill
...
flowchart TD
    ...
    G2 -->|通过| H{用户审查设计文档?}
    H -->|"要求修改"| F
    H -->|批准| I((调用 plan-and-split skill))
```

**终止状态是调用 plan-and-split。** 不要调用 `frontend-design`、`mcp-builder` 或任何其他实现类 skill。`think-and-design` 后唯一调用的 skill 是 `plan-and-split`。
