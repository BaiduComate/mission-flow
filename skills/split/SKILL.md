...
使用 question 工具等待用户确认，问题必须包含三个选项：

- 确认拆分：进入读取 iCafe 偏好和创建卡片流程
- 调整拆分：在 Other 中说明需要调整的内容，修改拆分结果后重新展示
- 跳过拆卡：不创建 iCafe 卡片，直接进入「询问是否编写计划文档」

若用户选择确认，进行下面读取 iCafe 偏好以及创建卡片流程。若用户选择调整，修改拆分结果并重新展示；用户确认前不要创建卡片。若用户选择跳过拆卡，跳过 iCafe 偏好读取和全部卡片创建步骤，直接进入“询问是否编写计划文档”；此时拆分结果仍作为后续 `plan` / `direct-impl` 的上下文，但提交不再绑定 iCafe 卡片。
